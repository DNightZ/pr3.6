  ...
  player1.attacks.forEach(attackData => {
    ...
    button.addEventListener('click', () => {
      if (attackCounter()) {
        ...
        if (isGameOver(player1, player2)) return;
...
function isGameOver(player1, player2) {
    if (player1.damageHP <= 0) {
        alert('Game Over! Charmander wins!');
        disableAllButtons();
        return true;
    } else if (player2.damageHP <= 0) {
        alert('Game Over! Pikachu wins!');
        disableAllButtons();
        return true;
    }
    return false;
...
// Функція для відключення всіх кнопок
